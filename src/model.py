...
class EsrganGenerator(nn.Module):
    def __init__(self, num_rrdb_blocks: int = 16):
        ...
        trunk = []
        for _ in range(num_rrdb_blocks):
            trunk += [ResidualInResidualDenseBlockEsrgan(channels=64, growth_channels=32, scale_ratio=0.2)]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out2 = self.conv2(trunk)
        out = torch.add(out1, out2)
        out = F.leaky_relu(self.up1(F.interpolate(out, scale_factor=2, mode="nearest")), negative_slope=0.2,
                           inplace=True)
        out = F.leaky_relu(self.up2(F.interpolate(out, scale_factor=2, mode="nearest")), negative_slope=0.2,
                           inplace=True)
        out = self.conv3(out)
        out = self.conv4(out)
...
class ResidualInResidualDenseBlockEsrgan(nn.Module):
    r"""The residual block structure of traditional ESRGAN and Dense model is defined
    Args:
        channels (int): Number of channels in the input image. (Default: 64)
        growth_channels (int): how many filters to add each layer (`k` in paper). (Default: 32)
    ...
    def __init__(self, channels: int = 64, growth_channels: int = 32, scale_ratio: float = 0.2):
        super(ResidualInResidualDenseBlockEsrgan, self).__init__()
        self.RDB1 = ResidualDenseBlock(channels, growth_channels, scale_ratio)
        self.RDB2 = ResidualDenseBlock(channels, growth_channels, scale_ratio)
        self.RDB3 = ResidualDenseBlock(channels, growth_channels, scale_ratio)
